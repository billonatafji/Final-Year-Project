...
def run_sim():
  ...
  gui_variables = {
    ...
    "accumulate_duration"     : int(request.form['accumulate_duration'] if request.form["accumulate_duration"] != "" else 0),
    "max_queue_lengths"       : {},
    "max_time_delays"         : {}
  }

  for inflow_edge in inflow_edges:
    gui_variables["max_queue_lengths"][inflow_edge] = int(request.form[inflow_edge+"0"]) if request.form["mode"] != "test_static" else 1
    gui_variables["max_time_delays"][inflow_edge] = int(request.form[inflow_edge+"1"]) if request.form["mode"] != "test_static" else 1
