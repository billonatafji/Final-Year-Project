...
class Simulation(object):
    ...
    def get_reward(self):
        ...
        for road in self.roads:
            queue_length_reward += (sum(road.get_previous_queue_lengths()) - sum(
                road.get_queue_lengths()))  # /road.get_max_allowed_queue_length()) #+ (1 if sum(road.get_overflows()) > 0 else 0)
            time_delay_reward += (sum(road.get_previous_time_delays()) - sum(
                road.get_time_delays()))  # /road.get_max_allowed_time_delay()
...
class ExperienceReplay(object):
    ...
    def get_batch(self, model, batch_size=10):
        ...
        env_dim = self.memory[0][0][0].shape[
            1]  # This is the size of the state vector in other words it is the input size to the model
        inputs = np.zeros((min(len_memory, batch_size), env_dim))  # Create empty batches of input states
        targets = np.zeros((inputs.shape[0],
                            num_phases))  # each input state in the batch will have rewards associated to phase choices
        for i, idx in enumerate(np.random.randint(0, len_memory, size=inputs.shape[
            0])):  # inputs.shape[0] is the number of elements in inputs / Here we are generating the batch
            ...
            inputs[
            i:i + 1] = old_state  # We insert the values from the batch to the inputs array to use in batch training
            # There should be no target values for actions not taken.
            # Thou shalt not correct actions not taken #deep

            targets[i] = model.predict(old_state)[
                0]  # Get from the model the rewards expected for the actions given each input state in inputs
            Q_sa = np.max(model.predict(state)[0])  # Get the maximum expected reward for the future state

            targets[
                i, phase] = reward + self.discount * Q_sa  # If the lane did not overflow set the reward as sum of immediate reward and discounted future reward
...
def train(shared_variables):
    ...
    roads = [
        Road("zeytouna-ain_mreysse", 100, 100,
             [shared_variables["lanes"]["zeytouna-ain_mreysse_0"], shared_variables["lanes"]["zeytouna-ain_mreysse_1"],
              shared_variables["lanes"]["zeytouna-ain_mreysse_2"],
              shared_variables["lanes"]["zeytouna-ain_mreysse_3"]]),
        Road("aub-ain_mreysse", 100, 100,
             [shared_variables["lanes"]["aub-ain_mreysse_0"], shared_variables["lanes"]["aub-ain_mreysse_1"]]),
        Road("bliss-ain_mreysse", 100, 200,
             [shared_variables["lanes"]["bliss-ain_mreysse_0"], shared_variables["lanes"]["bliss-ain_mreysse_1"]])
    ]
    ...
    while not shared_variables["doneEvent"].is_set():
        ...
        shared_variables[
            "new_phase"] = env.get_current_phase()  # if not shared_variables["accumulation"] or accumulation_counter > shared_variables["accumulate_duration"] else shared_variables["accumulate_phase"]
